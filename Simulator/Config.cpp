...
void Config::getConfigData(ifstream& fin)
{
	...
	int parsedBlockSize;
	fin >> word;
	parsedBlockSize = atoi(word.c_str());
	if (parsedBlockSize < 1)
	{
		cout << "ERROR: invalid block size" << endl;
		return;
	}
	word.clear();
	int tempBlockSize = convertToKilobytes(parsedBlockSize, dataConvType);
	if (tempBlockSize > systemMemory)
	{
		cout << "ERROR: invalid block size" << endl;
		return;
	}
	...
	word.clear();
	fin >> word;
	int parsedProjQuant = atoi(word.c_str());
	if (parsedProjQuant < 1)
	{
		cout << "ERROR: invalid projector quantity" << endl;
		return;
	}
	...
	word.clear();
	fin >> word;
	int parsedHddQuant = atoi(word.c_str());
	if (parsedHddQuant < 1)
	{
		cout << "ERROR: invalid hard drive quantity" << endl;
		return;
	}
...
*	Description: returns the memory block size in kbytes specified in the 
*		configuration file
*/
...
/**
*	Function: getHddQuant
*	Description: returns hard drive quantity specified in the configuration file
*/
int Config::getHddQuant()
{
	return hddQuant;
}

/**
*	Function: getProjQuant
*	Description: returns projector quantity specified in the configuration file
*/
int Config::getProjQuant()
{
	return projQuant;
}
...
*	Description: sets the memory block size to the parameter: source
*/
void Config::setBlockSize(int source)
{
	blockSize = source;
}

/**
*	Function: setHddQuant
*	Description: sets the hard drive quantity to the parameter: source
*/
void Config::setHddQuant(int source)
{
	hddQuant = source;
}

/**
*	Function: setProjQuant
*	Description: sets the projector quantity to the parameter: source
*/
void Config::setProjQuant(int source)
{
	projQuant = source;
}
...
/**
*	Function: convertToKiloytes
*	Description: converts data from dataType to kilobytes
*/
